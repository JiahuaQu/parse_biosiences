...
adata.raw = adata
#adata = adata[:, adata.var.highly_variable].copy()   # Do not subset
...
# Ensure the relative figures directory exists
figures_path = os.path.join('figures')
os.makedirs(figures_path, exist_ok=True)

# Plot and save the first histogram
plt.figure(figsize=(8, 6))
plt.hist(ol_expression, bins=30, color='skyblue', edgecolor='black')
plt.xlabel('OLAH Expression Level')
plt.ylabel('Frequency')
plt.title('Histogram of OLAH Expression')
plt.savefig(os.path.join(figures_path, 'OLAH_histogram.png'), bbox_inches='tight', dpi=300)
plt.savefig(os.path.join(figures_path, 'OLAH_histogram.pdf'), bbox_inches='tight')
plt.close()  # Close the figure after saving

# Plot and save the second histogram with different bin count
plt.figure(figsize=(8, 6))
plt.hist(ol_expression, bins=10, color='skyblue', edgecolor='black')
plt.xlabel('OLAH Expression Level')
plt.ylabel('Frequency')
plt.title('Histogram of OLAH Expression')
plt.savefig(os.path.join(figures_path, 'OLAH_histogram-2.png'), bbox_inches='tight', dpi=300)
plt.savefig(os.path.join(figures_path, 'OLAH_histogram-2.pdf'), bbox_inches='tight')
plt.close()  # Close the figure after saving
